...
try {
    $query = mysqli_query($connect, "SELECT * FROM content_management WHERE content_type='mw_franchisee_operation_policy' AND is_active=1");
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $page_title = 'MW Franchisee Operation Policy';
        $page_content = '<p>Content not available. Please contact administrator.</p>';
        $meta_description = 'MW Franchisee Operation Policy for MiniWebsite';
        $meta_keywords = 'franchisee, operation, policy, miniwebsite';
    }
} catch (Exception $e) {
    $page_title = 'MW Franchisee Operation Policy';
    $page_content = '<p>Content not available. Please contact administrator.</p>';

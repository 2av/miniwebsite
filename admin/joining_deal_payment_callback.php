<?php
/**
 * Joining Deal Payment Callback Handler
...
error_log("Joining Deal Payment Callback - Email: $user_email, Transaction: $transaction_id, Amount: $amount_paid, Deal ID: $joining_deal_id, Mapping ID: $mapping_id");
...
try {
    ...
    if ($stmt->execute()) {
        ...
        error_log("Joining deal payment successful: User: $user_email, Amount: $amount_paid, Transaction: $transaction_id");
        
        // Create invoice entry in invoice_details table
        $invoice_query = "INSERT INTO invoice_details (
            ...
            mapping_id,
            created_at
        ) VALUES (?, ?, ?, 'PAID', NOW(), 'JOINING_DEAL', ?, ?, NOW())";
        ...
        if ($invoice_stmt->execute()) {
            ...
            // Update start_date and expiry_date in user_joining_deals_mapping
    ...
    error_log("Joining deal payment callback error: " . $e->getMessage());

...
try {
    ...
    if($result['status']) {
        ...
        if(file_put_contents($upload_path, $result['data'])) {
            ...
            $stmt = $connect->prepare("SELECT image FROM user_details WHERE email = ? AND role = 'TEAM' LIMIT 1");
            ...
            $stmt = $connect->prepare("UPDATE user_details SET image = ? WHERE email = ? AND role = 'TEAM'");
            if($stmt) {
                ...
                if($stmt->execute()) {
                    ...
                    if($affected_rows == 0) {
                        ...
                        $check_stmt = $connect->prepare("SELECT id FROM user_details WHERE email = ? AND role = 'TEAM' LIMIT 1");
                        ...
                        if($check_result->num_rows == 0) {
                            ...
                            $insert_stmt = $connect->prepare("INSERT INTO user_details (email, role, image) VALUES (?, 'TEAM', ?)");

...
<?php
if($row['d_payment_status']=='Created' && $row['complimentary_enabled'] == 'No'){echo '<div class="card_status1"><p>Trial</p></div>';
?>
<div class="save_contact_popup">
	...
	<p>Your miniwebsite has been created.</p>
	<p>Pay <del> ₹ 1999 </del>  ₹999 now to activate your miniwebsite.</p>
...
<!----------------------copy from here ------------------------->
	...
	<div class="qr_code_container">
		...
		$qrCodeUrl = "https://" . $_SERVER['HTTP_HOST'] . "/" . $row['card_id'];
		$encodedUrl = urlencode($qrCodeUrl);
		// Use a more reliable QR code service
		$qrImageUrl = "https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=" . $encodedUrl;
		?>
		<img src="<?php echo $qrImageUrl; ?>" id="qr_code_d" alt="QR Code">

		<div class="download-btn-wrapper">
			<a href="../../generate-qr.php?card_id=<?php echo $row['card_id']; ?>" class="qr_download_btn">
	...
	<div class="card2" id="shop_online">
		...
		for($x=0;$x<=20;$x++){
			if(!empty($row3["pro_name$x"])){
				...
				echo "<a href='https://$_SERVER[HTTP_HOST]/order_online/checkout.php?pro_id=$x&card_id=$row[id]' target='_blank'><div class='btn_buy'>Buy Now</div></a>";
				
				echo "<a href='https://api.whatsapp.com/send?phone=91".str_replace("+91","",$row['d_whatsapp'])."&text=I am interested in Product: ".$row3["pro_name$x"].", Price: ".$row3["pro_mrp$x"]."' target='_blank'><div class='btn_buy'>Buy Now</div></a>";

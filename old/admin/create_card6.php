...
for ($m=1;$m <= 10; $m++){
	...
	<div class="divider"><div class="num"><?php  echo "$m"; ?></div>
		...
		<img src="<?php if(!empty($row2["d_gall_img$m"])){echo 'data:image/*;base64,'.base64_encode($row2["d_gall_img$m"]);}else {echo '../panel/images/upload.png';} ?>" alt="Select image" alt="Select image" id="<?php  echo "showPreviewLogo$m"; ?>" onclick="<?php  echo "clickFocus($m)"; ?>" >
		<div class="input_box">
		...
		for($x=0; $x<=10; $x++) {
			if(!empty($_FILES["d_gall_img$x"]['tmp_name'])) {
				...
				if($_FILES["d_gall_img$x"]['size'] <= 250000) {
					...
					echo '<div class="alert danger">File size for Gallery Image '.($x+1).' exceeds 250KB limit. Please resize your image.</div>';
				}
			}
		}

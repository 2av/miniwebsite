...
$('#uploadCroppedImage').on('click', function() {
    ...
    canvas.toBlob(function(blob) {
        ...
        var isFranchiseePage = currentPath.indexOf('/franchisee/') !== -1;
        var uploadUrl;
        
        // Build absolute URL to avoid path resolution issues
        // Extract the base path (everything before /customer/, /team/, or /franchisee/)
        ...
        } else if (isFranchiseePage) {
            folderIndex = pathParts.indexOf('franchisee');
        } else {
            folderIndex = pathParts.indexOf('customer');
        }
        
        if (folderIndex >= 0) {
            // Build base path up to and including the folder (team, customer, or franchisee)
        ...
        console.log('Is franchisee page:', isFranchiseePage);
